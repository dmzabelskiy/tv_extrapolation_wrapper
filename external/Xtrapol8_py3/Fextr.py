...
def run(args):
    ...
    maptypes_zip   = list(zip(all_maptypes, all_maps))
    ...
    params.input.additional_files = list([os.path.abspath(x) for x in params.input.additional_files])
    ...
    for mp in reversed_maptypes:
        ...
        if (params.f_and_maps.fast_and_furious == False and params.refinement.run_refinement):
            ...
            if pdb_list == recrealref_lst:
                ...
                if mp == 'qFgenick_map':
                    ccp4_list = [re.search("(.+?)2mqFgenick-DFc_reciprocal", fle).group(1)+"mqFgenick-DFc.ccp4" for fle in pymol_mtz_list]
                elif mp == 'kFgenick_map':
                    ccp4_list = [re.search("(.+?)2mkFgenick-DFc_reciprocal", fle).group(1)+"mkFgenick-DFc.ccp4" for fle in pymol_mtz_list]
                elif mp == 'Fgenick_map':
                    ccp4_list = [re.search("(.+?)2mFgenick-DFc_reciprocal", fle).group(1)+"mFgenick-DFc.ccp4" for fle in pymol_mtz_list]
                else:
                    ccp4_list = [re.search("(.+?)\_reciprocal", fle).group(1)+".ccp4" for fle in pymol_mtz_list]
