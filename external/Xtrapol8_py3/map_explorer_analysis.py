...
class Map_explorer_analysis(object):
    ...
    def get_ss(self, pdb_file):
        ...
        for chain in hier.chains():
            ...
            if ID not in list(chain_dict.keys()):
                ...
                helix_ranges = [list(range(lne[1], lne[2] + 1, 1)) for lne in helix_lst if ID in lne[0]]
                strand_ranges = [list(range(lne[1], lne[2] + 1, 1)) for lne in strand_lst if ID in lne[0]]
                try:
