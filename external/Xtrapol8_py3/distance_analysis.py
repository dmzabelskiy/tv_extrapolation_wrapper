...
class Distance_analysis(object):
    ...
    def __init__(self, pdblst, occupancies, resids_lst = None, plateau_fraction=0.99, use_waters = True, outsuffix = '', log = sys.stdout):
        ...
        alphas = list([round(1/x, 3) for x in occupancies])
        ...
        zipped = list(zip(alphas, pdblst))
        zipped_sorted = sorted(zipped, key = lambda x:x[0])
        alphas, pdblst =list(zip(*zipped_sorted))
    ...
    def extract_alpha(self):
        ...
        if possible_b.shape[0] > 0:
            ...
            mode = np.atleast_1d(scipy.stats.mode(np.round(possible_b, 2), keepdims=False).mode)[0]
            ...
            mode_x0 = np.atleast_1d(scipy.stats.mode(np.round(possible_x0, 2), keepdims=False).mode)[0]
